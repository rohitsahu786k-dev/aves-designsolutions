...
const revalidate = Number(process.env.NEXT_PUBLIC_REVALIDATE_SECONDS || 900);
...
export const getProduct = cache(async function getProduct(slug) {
  ...
  const [products, wpPosts] = await Promise.all([
    getProducts({ slug, per_page: "1" }, { next: { revalidate: 15 } }),
    wpFetch(`/wp-json/wp/v2/product?slug=${encodeURIComponent(slug)}&_fields=id,slug,acf`, { next: { revalidate: 15 } }).catch(() => []),
  ]);
...
export const getCategories = cache(async function getCategories(params = {}) {
  ...
  const categories = await wpFetch(`/wp-json/wc/store/v1/products/categories?${query}`, { next: { revalidate: 1800 } });
...
export async function getPostCategories(params = {}) {
  const query = new URLSearchParams({ per_page: "100", hide_empty: "false", ...params });
  const wpCats = await wpFetch(`/wp-json/wp/v2/categories?${query}`, { next: { revalidate: 60, tags: ["categories"] } }).catch(() => []);
...
export async function getPosts(params = {}) {
  ...
  const wpPosts = await wpFetch(`/wp-json/wp/v2/posts?${query}`, { next: { revalidate: 60, tags: ["posts"] } }).catch(() => []);
...
export async function getPost(slug) {
  if (!slug) return null;
  const wpPosts = await wpFetch(`/wp-json/wp/v2/posts?slug=${encodeURIComponent(slug)}&_embed=1`, { next: { revalidate: 60, tags: ["posts", `post-${slug}`] } }).catch(() => []);
