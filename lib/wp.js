...
export async function getPostCategories(params = {}) {
  const query = new URLSearchParams({ per_page: "100", hide_empty: "false", ...params });
  const wpCats = await wpFetch(`/wp-json/wp/v2/categories?${query}`, { next: { revalidate: 30 } }).catch(() => []);
...
export async function getPosts(params = {}) {
  ...
  const wpPosts = await wpFetch(`/wp-json/wp/v2/posts?${query}`, { next: { revalidate: 30 } }).catch(() => []);
...
export async function getPost(slug) {
  if (!slug) return null;
  const wpPosts = await wpFetch(`/wp-json/wp/v2/posts?slug=${encodeURIComponent(slug)}&_embed=1`, { next: { revalidate: 30 } }).catch(() => []);
