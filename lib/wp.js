...
import { SCREWNET_BLOGS, getAllBlogs, getBlogBySlug } from "@/lib/screwnet-blogs";
...
export async function getPosts(params = {}) {
  const wpPosts = await wpFetch(`/wp-json/wp/v2/posts?per_page=12&_embed=1`).catch(() => []);
  const customPosts = SCREWNET_BLOGS.map(mapScrewnetBlogToWpPost);
  if (Array.isArray(wpPosts) && wpPosts.length > 0) {
    return [...wpPosts, ...customPosts];
  }
  return customPosts;
}
