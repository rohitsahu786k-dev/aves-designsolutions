...
function commerceAuthHeader() {
  const key = process.env.WOOCOMMERCE_CONSUMER_KEY || process.env.WP_APPLICATION_USERNAME || "";
  const secret = process.env.WOOCOMMERCE_CONSUMER_SECRET || process.env.WP_APPLICATION_PASSWORD || "";
...
// Store API is public and returns prices in minor units — used to price-check the
// browser's cart before an order is created.
// Returns null when the store could not be reached, so callers can tell a
// transport failure apart from "these products do not exist".
export async function fetchStoreProducts(ids = []) {
  ...
  const response = await fetch(`${WP_URL}/wp-json/wc/store/v1/products?${query}`, { cache: "no-store" }).catch(() => null);
