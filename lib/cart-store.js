...
function checkoutBaseUrl() {
  return process.env.NEXT_PUBLIC_WP_URL || "https://wp.screwnet.in";
}
...
export function buildWooCommerceCheckoutUrl(items = [], coupon = null, redirect = "checkout") {
  ...
  const payload = {
    items: items.map((item) => ({
      ...
      variation_id: item.variationId || item.variation?.id || 0,
      variation: item.variationAttributes || (Array.isArray(item.variation?.attributes)
        ? Object.fromEntries(item.variation.attributes.map((attribute) => [
            attribute.taxonomy || attribute.name,
